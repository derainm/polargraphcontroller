...
Serial myPortt;  // Create object from Serial class
...
void draw()
{
  ...
  if (drawbotReady)
  {
    dispatchCommandQueue();
       ...
       String fileName = dataPath("diagonal.tif");
    File f = new File(fileName);
    if (f.exists()) {
      f.delete();
    }
    if (myPortt.available() > 0)
    {  // If data is available,
      //prec = val;
      val = myPortt.readStringUntil('.');         // read it and store it in val
      size(1366, 768);   
  ...
  // Saves a TIFF file named "diagonal.tif"
    File ddf=sketchFile("C:\\Users\\m\\Desktop\\e script\\067-XY-Plotter-Drawing-Robot\\Polargraph 1.2\\processing-source\\polargraphcontroller\\diagonal.tif");
    ...
    if(ddf.exists())
    {
      PImage img = loadImage("C:\\Users\\m\\Desktop\\e script\\067-XY-Plotter-Drawing-Robot\\Polargraph 1.2\\processing-source\\polargraphcontroller\\diagonal.tif");
            if (img != null) 
            {
              img = null;
              getDisplayMachine().loadNewImageFromFilename("C:\\Users\\m\\Desktop\\e script\\067-XY-Plotter-Drawing-Robot\\Polargraph 1.2\\processing-source\\polargraphcontroller\\diagonal.tif");
